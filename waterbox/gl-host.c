...
/* Which context the calls are landing on: GL_OP_CONTEXT_ID, issue #43. Zero
 * until one is made, and zero is the contract's "cannot tell". */
...
static int create_context(char *err, int errlen)
{
	...
	/* Dolphin's renderer targets CORE profile (its loader treats every
	 * legacy compatibility entry point as required otherwise), so ask for
	 * one explicitly: 4.5 first, 3.3 as the floor its backend documents. */
	{
		...
		if (s_context == EGL_NO_CONTEXT)
			s_context = eglCreateContext(s_display, config, EGL_NO_CONTEXT, core33);
	}
	if (s_context == EGL_NO_CONTEXT)
	{
 ...
 * video.rebuildOnStateLoad false. RPCS3 declares nothing, so it rebuilds. */
 ...
 * did that for. run-wbx and run-native say this out loud at the end of a run
 * and the gate fails any leg it is not zero for. */
...
int chimera_gl_host_init(char *err, int errlen)
{
	...
	/* Loaded; now let go. A context is current on ONE thread at a time, and
...
#ifndef _WIN32
static __thread int t_bound;

/* Release the context from the calling thread so another may claim it (EGL
 * allows a context current on one thread at a time). The single-binary
 * native reference needs this between its main-thread probe and the
 * emulator thread's real work; a sandboxed guest calls from one host thread
 * and never needs it. */
...
/* Bind the context to the calling thread. The dispatcher does this lazily
 * for a sandboxed guest; a native reference whose renderer calls the driver
 * directly (no dispatcher in between) binds from its own GL thread. */
int chimera_gl_host_bind_current(void)
{
	...
	if (!t_bound)
	{
		EGLBoolean ok = eglMakeCurrent(s_display, s_surface, s_surface, s_context);
		if (getenv("CHIMERA_TRACE_GL"))
			fprintf(stderr, "[glhost] bind on thread: ok=%d err=0x%x\n", (int)ok, eglGetError());
		if (!ok)
			return -1;
		t_bound = 1;
	}
                                              ...
                                              uintptr_t c, uintptr_t d, uintptr_t e)
{
...
#ifndef _WIN32
	/* The machine may call from a thread that never bound the context (a
	 * native reference runs its GL on the emulator's own thread); "current
	 * context" is per-thread state, so bind lazily, once per thread. */
	if (!t_bound && s_context != EGL_NO_CONTEXT)
	{
		EGLBoolean ok = eglMakeCurrent(s_display, s_surface, s_surface, s_context);
		if (getenv("CHIMERA_TRACE_GL"))
			fprintf(stderr, "[glhost] bind on thread: ok=%d err=0x%x\n", (int)ok, eglGetError());
	...
	switch (op)
	{
		...
		case GL_OP_CONTEXT_ID:
			 ...
			 * (GLGSRender::chimera_check_gl_context) and rebuilds them when it
			 * moves. Answering 0, which is what this host did before it had
			 * this case, means "cannot tell" - and a renderer that cannot tell
			 * must assume nothing moved, so it kept the dead session's names.
			 * That is the whole of issue #43, back again, in the one harness
			 * that exists to prove it fixed. */
...
#include "gl-bridge-host.inc"
		...
		default:
			 ...
			 * for as long as the opcode existed; gpu:flip PASSED while this
			 * line printed 365 times in a single 60-frame run, and gpu:context
			 * - the leg that exists for issue #43 - could not have passed at
			 * all. Absent was indistinguishable from working
			 ...
			 * because 365 lines a run is how this one stayed invisible; the
			 * count is not. */
			s_unhandled++;
