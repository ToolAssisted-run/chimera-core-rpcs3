...
cmake -G Ninja -B "$root/build/native" $RPCS3_OPTS -DLLVM_DIR="$root/build/llvm-native/lib/cmake/llvm" -DUSE_SYSTEM_FFMPEG=ON -DFFMPEG_INCLUDE_DIR="$ffmpeg_inc" -DFFMPEG_LIBRARIES=avcodec -DCMAKE_CXX_FLAGS="-DCHIMERA_CORE -I$here" "$root/extern/rpcs3"
