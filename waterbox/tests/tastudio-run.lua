-- Frontend witness for the RPCS3 package: run the core inside Chimera for a
-- fixed number of frames with nothing pressed, then dump the start of main memory.
-- The driver compares that dump byte-for-byte against the native reference
-- (run-native --dump-domain).
--
-- A slice rather than the whole domain, deliberately: main memory is 256 megabytes,
...
-- TAStudio opens paused, and a paused TAStudio does not step for a script's
-- frameadvance. A seek is what the user does, so it is what this does: ask
-- for the frame, then yield until the machine is there (or gives up: the
-- runner outside kills a run whose clock stops).
if tastudio.engaged() then
	tastudio.setplayback(frames)
	local spins = 0
	while emu.framecount() < frames do
		emu.frameadvance()
		spins = spins + 1
		if spins % 100 == 0 then
			console.log(string.format("seek: at frame %d of %d, lag %d", emu.framecount(), frames, emu.lagcount()))
		end
	end
else
	for _ = 1, frames do
